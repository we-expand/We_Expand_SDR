...
- **Testado no preview do browser (depois, mesma sessão)**: outra janela estava com o dev server na porta 5173, então ajustei `vite.config.js` (`server.port` respeita `process.env.PORT`) e `.claude/launch.json` (`autoPort: true`, tanto o da raiz `we-expand/.claude/launch.json` quanto o de `sdr/.claude/launch.json`) pra conseguir subir uma instância própria numa porta livre. Testei o fluxo completo na aba "Conexões do LinkedIn (CSV)": upload de um CSV de teste com 5 linhas no formato real do export (preâmbulo "Notes:" + header `First Name,Last Name,URL,Email Address,Company,Position,Connected On`), barra de progresso mostrou "5 de 5 conexões processadas...", os 5 leads apareceram em "Meus leads" sem score (como esperado). Depois apaguei os 5 registros de teste pela própria UI (seleção múltipla + excluir), confirmando que o total voltou de 149 pra 144 — dados reais do Cleber preservados.
- **Deploy**: confirmado que a pasta `sdr` já tem `.git` inicializado (feito em outra janela), remote `origin` apontando pra `https://github.com/we-expand/We_Expand_SDR.git`, branch `main`, e já está vinculada a um projeto Vercel (`.vercel/project.json`: `we-expand-sdr`, projectId `prj_MUu8bcMO8wlaAVF9NfoMShHI748b`). Passei ao Cleber os comandos de `git add`/`commit`/`push` pra essas mudanças (não executados por mim, ver [[feedback_git_vercel_sync]]) e alertei que as env vars (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `GROQ_API_KEY`, `APOLLO_API_KEY`) precisam estar configuradas no painel da Vercel também, senão o deploy sobe mas o backend quebra em produção — ainda não confirmei com ele se isso já foi feito.
